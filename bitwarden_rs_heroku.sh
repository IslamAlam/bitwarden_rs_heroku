...
function heroku_bootstrap {
    ...
    APP_NAME=$(heroku create --region eu "${CREATE_APP_NAME}" --json | jq --raw-output '.name')
    ...
    heroku config:set DATABASE_MAX_CONNS="${DATABASE_CONNS}" -a "${APP_NAME}"
}
...
while getopts a:b:c:d:g:t:u:v: flag
do
    case "${flag}" in
